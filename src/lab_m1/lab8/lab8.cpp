...
void Lab8::Init()
{
    ...
    point_light_pos[1] = glm::vec3(0, 1, 3);
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int spot_light_loc = glGetUniformLocation(shader->program, "is_spot_light");
    glUniform1i(spot_light_loc, is_spot_light);
    ...
    // Bind model matrix
    //auto ct = glGetUniformLocation(shader->program, "Model");
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_F)) {
        if (is_spot_light == 0) is_spot_light = 1;
        else is_spot_light = 0;
    }
